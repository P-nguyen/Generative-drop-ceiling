...
class Point_Check:
	...
	def objs_by_proximity(self, geometry_list): # this code will find the relative objects with in 4' and set the lowest pt as your standard Z
		...
		for obj in geometry_list:
			...
			distance = closest_point.distance_to(test_point)
			if distance > 48:
				continue
	...
	def point_adjustment(self): #???
		...
		#self.check = closest_point
		#while (distance < 12) or (distance > 17):			
		if distance < 12:
			z_change = (math.sqrt((12**2)-(distance**2)))
			self.point.set_z( self.point.z() - z_change)
			
		if distance > 17: 
			#self.point.set_z( self.point.z() + distance/2)
			test_point = self.point
			test_point.set_z(closest_point.z())
			test_distance = closest_point.distance_to(test_point)
			z_change = (math.sqrt((12**2)-(test_distance**2)))
...
for i in range(int(x_number) + 1):
		for j in range(int(y_number) + 1):
			...
			#checkpts.append( grid[i][j].check)
...
OUT = [testpt, checkpts] #grid_points #grid_points[0].boolean