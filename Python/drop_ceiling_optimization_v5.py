...
class PointCheck:
	...
	def objects_within_proximity( self, geometry_list ):	#stores whatever ceiling objects are with in proximity
		...
		for object in geometry_list:
			...
			if lowest_z == None:
				...
				lowest_z = test_point.z()
				
			#if dist <= self.closest_geometry_distance and test_point.z() <= lowest_z:
	...
	def set_closest_geometry( self, geometry_list ):  #this one only finds the closest point. we need one that finds the proximitiy.
		...
		#L.sort(key=itemgetter(1)) py> L.sort(key=lambda sublist: sublist[1])
		#self.stored_geometry.sort(key=lambda sublist: sublist[1], reverse=True)
		
		#for object in self.stored_geometry:
		#	object_point = object.get_closest_point(self.point)
...
for i in range(int(x_number) + 1):
		for j in range(int(y_number) + 1):
			...
			#grid[i][j].objects_within_proximity(reference_geo)
			grid[i][j].set_top_point()
			output_debug_geometry.append(grid[i][j].stored_geometry)
...
OUT = output_debug_geometry #[output_debug_geometry, output_debug_new]
